...
class NonBonded(BaseNonBonded):

    title='Non-bonded potential'
    ...
    def __init__(self):
        ...
        self.widget = widgets.VBox(children=[hbox1, hbox3, self.titlebox, self.g])

    @property
    def y(self):
        return self.calculate(self.eps_slider.value, self.sigma_slider.value, 
                              self.qa_slider.value, self.qb_slider.value, self.x)


nb_potential = NonBonded()
